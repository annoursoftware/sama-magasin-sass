...
class IngredientController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = DB::table('ingredients as i')
            ->join('categories as c','i.categorie_id','=','c.id')
            /* ->leftJoin('users as u','b.id','=','u.boutique_id') */
            ->select('i.*', 'c.categorie'/* DB::raw('COUNT(u.id) as total_users'), */
            )
            ->get();
        ...
        if ($request->ajax()) {    
            ...
            ->addColumn('Image', function ($row) {
                if (is_null($row->image)) {
                    ...
                    return '<a href="' . url("/upload/Ingredients/Images/".$row->image).'" data-toggle="lightbox" data-title="'.$row->ingredient.'" data-footer="ingredient-'.$row->image.'" data-max-width="500">
                                <img class="rounded-square" width="30" height="30" src="' . asset("/upload/Ingredients/Images/".$row->image).'" alt="'.$row->image.'">
                            </a>';
                }
            })
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'ingredient.required' => "L'ingredient est obligatoire.",
        ...
        if ($request->hasFile('image')) {
            ...
            $destination = public_path() . "/upload/Ingredients/Images/";
            $file->move($destination, Str::slug($request->ingredient).'-'.Str::slug($recherche_boutique->boutique).'-'.time().'.'.$file->getClientOriginalExtension());
            $data->image = Str::slug($request->ingredient).'-'.Str::slug($recherche_boutique->boutique).'-'.time().'.'.$file->getClientOriginalExtension();
        } else {
            $data->image = null;
        }
        ...
        return response()->json([
            'success' => 'Ingredient enregistré avec succès !',
            'data' => $data,
            'status' => 201,
        ]);
    }
    ...
    {
        $data = DB::table('ingredients as i')
            ->join('categories as c','i.categorie_id','=','c.id')
            ->join('boutiques as b','a.boutique_id','=','b.id')
            ->select('i.*', 'c.categorie', 'b.boutique')
            ->where('i.id', $id)
            ->first();

        if ($request->ajax()) {
            return response()->json([
                'success' => 'Ingredient chargé avec succès !',
                'data' => $data,
                'status' => 200,
            ]);
        }
    ...
    public function update(Request $request, string $id)
    {
        ...
        return response()->json([
            'success' => 'Ingredient modifié avec succès !',
            'data' => $data,
        ]);
    }
    ...
    public function destroy(string $id)
    {
        ...
        return response()->json(['success' => 'Ingredient supprimé avec succès !']);
    }
