                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            ...
                            <!-- /.card-body -->
                            @include('back.produits.form')
                            @include('back.produits.show')
                        </div>
                        <!-- /.card -->
                    </div>
                    <!-- /.col -->
...
<script>
    ...
    <script>
        ...
        $('#formulaire').submit(function (e) {
            ...
            var id = $('#id').val();
            if(save_method == 'add')
            url = "{{ url('api/produits') }}";
            else
            url = "{{ url('api/produits') . '/' }}" + id;
        ...
        function showData(id){
            $.ajax({
                ...
                success: function(row){
                    ...
                    $('.modal-title-show').text('Visualisation produit');

                    $('#id-show').val(row.data.id);
                    /* $('.info-box-number').html(data.role); */
                    $('#produit-show').html(row.data.produit);
                    ...
                    if (row.data.image==null) {
                        ...
                        $('#logo-show').attr('src', '{{ asset("upload/Produits/Images") }}'+'/'+row.data.image);
        ...
        function editData(id){
            ...
            $.ajax({
                ...
                success: function(row){
                    ...
                    console.log(data);
                    $('#modal-form').modal('show');
                    $('.modal-title').text('Modification Produit');
                    $('#insertbtn').text(' Modifier');

                    $('#id').val(data.id);
                    $('#produit').val(row.data.produit);
