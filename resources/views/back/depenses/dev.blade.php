                        ...
                        <div class="info-box shadow-lg">
                            ...
                            <div class="info-box-content">
                                ...
                                <span class="info-box-number">
                                    {{ number_format($nb_depenses, 0, ",",".") }} / {{ number_format($mt_depenses, 0, ",",".") }}
                                </span>
                            </div>
                            <!-- /.info-box-content -->
                        </div>
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            ...
                            <!-- /.card-body -->
                            @include('back.depenses.form')
                            @include('back.depenses.show')
                        </div>
                        <!-- /.card -->
                    </div>
                    <!-- /.col -->
    ...
    <script>
        $(function () {
            var table = $('#table').DataTable({
                ...
                autoWidth: true,
                responsive: true, */
                ajax: "{{ route('depenses.index') }}",
                columns: [
                    {data: 'id', name: 'id'},
                    {data: 'num_depense', name: 'num_depense'},
                    {data: 'numero_facture_benef', name: 'numero_facture_benef'},
        ...
        function addForm() {
            var url = window.location.origin+'/admin/autres-depenses/depense/nouveau/';
        ...
        $('#formulaire').submit(function (e) {
            ...
            $.ajax({
                ...
                processData: false,
                success: (response) => {
                    $('#nv_depense').html('Enregistrement');
        ...
        function showData(id){
            $.ajax({
                ...
                success: function(row){
                    ...
                    $('.modal-title-show').text('Visualisation Dépense');

                    $('#id-show').val(row.data.id);
                    /* $('.info-box-number').html(data.role); */
                    $('#libelle-show').html(row.data.libelle);
                    $('#numero_facture_benef-show').html(row.data.numero_facture_benef);
                    $('#montant-show').html(row.data.montant);
                    $('#type-show').html(row.data.type);
                    $('#effet-show').html(row.data.effet);
                    $('#limite-show').html(row.data.limite);
        ...
        function editData(id){
            ...
            $.ajax({
                ...
                success: function(row){
                    ...
                    $('.modal-title').text('Modification dépense');
                    $('#nv_beneficiaire').text(' Modifier');

                    $('#id').val(data.id);
                    $('#libelle').html(row.data.libelle);
                    $('#numero_facture_benef').html(row.data.numero_facture_benef);
                    $('#montant').html(row.data.montant);
                    $('#type').html(row.data.type);
                    $('#effet').html(row.data.effet);
                    $('#limite').html(row.data.limite);
                    $('#etat').html(row.data.etat);
                    $('#beneficiaire').html(row.data.beneficiaire_id);
                },
                error: function(){
                    const Toast = Swal.mixin({
                        toast: true,
