                ...
                <div class="row mb-2">
                    ...
                    <div class="col-md-12 col-sm-12 col-12 d-flex">
                        <button type="button" class="btn btn-flat btn-primary ml-auto" onclick="addForm()">
                            <i class="bi bi-plus-lg"></i> Ajouter une Boutique
                        </button>
                    </div>
                </div>
                <!-- /.container-fluid -->
...
@push('scripts')
    ...
    <script>
        ...
        function addForm() {
            ...
            $('.modal-title').text('Nouveau boutique');
            $('#id').val('');
        ...
        $('#formulaire').submit(function (e) {
            ...
            var id = $('#id').val();
            if(save_method == 'add')
            url = "{{ url('api/boutiques') }}";
            else
            url = "{{ url('api/boutiques') . '/' }}" + id;
        ...
        function showData(id){
            ...
            $.ajax({
                ...
                success: function(row){
                    ...
                    $('.modal-title-show').text('Visualisation boutique');

                    $('#id-show').val(row.data.id);
                    /* $('.info-box-number').html(data.role); */
                    $('#entreprise-show').val(row.data.entreprise);
                    $('#telephone-show').val(row.data.telephone);
        ...
        function editData(id){
            ...
            $.ajax({
                ...
                success: function(row){
                    ...
                    console.log(data);
                    $('#modal-form').modal('show');
                    $('.modal-title').text('Modification Boutique');
                    $('#insertbtn').text(' Modifier');

                    $('#id').val(data.id);
                    $('#boutique').val(data.boutique);
